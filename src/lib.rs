...
impl Simulation{
    ...
    pub fn step(&mut self){
        ...
        Simulation::enforce_zero_neumann(&mut self.temperature);
        Simulation::enforce_zero_neumann(&mut self.phi);
...
// These functions are not exposed to wasm
impl Simulation{
    // Enforces homogeneous Neumann boundaries on all four edges for the input field `field`
    pub fn enforce_zero_neumann(field: &mut Array2D){
        for j in 0..field.size[1]{
            let index_left  = field.ravel(0, j);
            let index_right = field.ravel(field.size[0] - 1, j);

            let value_left = field.value(2, j);
            let value_right = field.value(field.size[0] - 3, j);

            field.data[index_left] = value_left;
            field.data[index_right] = value_right;
        }

        for i in 0..field.size[0]{
            let index_bottom  = field.ravel(i, 0);
            let index_top = field.ravel(i, field.size[1] - 1);

            let value_bottom = field.value(i, 2);
            let value_top = field.value(i, field.size[1] - 3);

            field.data[index_bottom] = value_bottom;
            field.data[index_top] = value_top;
        }
    }
}
